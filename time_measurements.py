...
def measure_time(searching_element: int=None, experiment_name: str=None):
    ...
    for array_size in array_sizes:
        ...
        if not searching_element:
            searching_element = np.random.choice(array)

        for algorithm in algorithms:
            timer.start_measuring()
            try:
                algorithm(array, searching_element).get_result()
            except Exception:
                pass
